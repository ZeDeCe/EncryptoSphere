...
class GoogleDrive(CloudService):
    ...
    def download_file(self, file_name: str, path: str):
        ...
        """
        try:
            # Search file by name - need to change to path!!!
            results = self.drive_service.files().list(
                q=f"name = '{file_name}'",
    ...
    def delete_file(self, file_name: str, path: str) -> bool:
        ...
        """
        try:
            query = f"name = '{file_name}' and '{path}' in parents"
    ...
    def create_folder(self, folder_path: str):
        """
        Create folder on Google Drive and return its path.
        If the folder already exists, return its existing path.
        """
        try:
            ...
            if folders:
                ...
                return f"/{existing_folder['name']}"  # מחזירים את הנתיב של התיקייה הקיימת
            ...
            return f"/{new_folder['name']}"  # מחזירים את הנתיב של התיקייה החדשה
