...
from dropbox.sharing import MemberSelector, AddMember
import webbrowser
import os 
...
def share_folder(folder_path, email ,access_token):
    dbx = dropbox.Dropbox(access_token)
    try:
        if not folder_path.startswith('/'):
            folder_path = '/' + folder_path
        # Create a member to be added (email)
        member = AddMember(MemberSelector.email(email))
        
        # Share the folder with the member (email address)
        result = dbx.sharing_add_folder_member(folder_path, [member])
        
        # Return a success message
        return f"Folder shared successfully with {email}!"
    except ApiError as e:
        print(f"Error sharing folder '{folder_path}' with {email}: {e}")
        return None
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            recipient_email = input("Enter email to share: ")
            share_folder(dropbox_folder_path,recipient_email ,access_token)
        elif choice == '8':
            print("Exiting...")
            break
        else:
